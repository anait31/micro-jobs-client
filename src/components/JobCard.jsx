...
const JobCard = ({ job }) => {
    const { category, job_title, _id, deadline, description } = job || {};
    return (
        <div className="w-full max-w-sm px-4 py-3 bg-white rounded-md shadow-md dark:bg-gray-800">
            ...
            <Link to={`/jobs/${_id}`}>
                <div className="flex items-center justify-between">
                    ...
                    <span className="px-3 py-1 text-xs text-blue-800 uppercase bg-blue-200 rounded-full dark:bg-blue-300 dark:text-blue-900">{deadline}</span>
                </div>
                <div>
                    <h1 className="mt-2 text-lg font-semibold text-gray-800 dark:text-white">{job_title}</h1>
                    <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">{description?.slice(0, 50)} ...</p>
                </div>
            </Link>
            <div>
